...
external interface TestController {
   ...
   * turn have their own [TestItem.children children] to form the
   * "test tree."
   *
  ...
  fun createRunProfile(
    ...
    runHandler: (request: TestRunRequest, token: CancellationToken) -> Any, // Thenable<void> | void
   ...
   */
  var resolveHandler: ((item: TestItem?) -> Any /* Thenable<void> | void */)?
   ...
   */
  var refreshHandler: ((token: CancellationToken) -> Any /* Thenable<void> | void */)?
   ...
   * may be used to drive [TestRunRequest.continuous continuous test runs].
   *
