...
external interface TreeDragAndDropController<T> {
  /**
   * The mime types that the [TreeDragAndDropController.handleDrop] method of this `DragAndDropController` supports.
   ...
   *
   * Use the special `files` mime type to support all types of dropped [DataTransferFile]s, regardless of the file's actual mime type.
   *
  ...
  /**
   * The mime types that the [TreeDragAndDropController.handleDrag] method of this `TreeDragAndDropController` may add to the tree data transfer.
   * This could be well-defined, existing, mime types, and also mime types defined by the extension.
   *
   ...
   */
  fun handleDrop(target: T?, dataTransfer: DataTransfer, token: CancellationToken): Any // Thenable<void> | void
}
